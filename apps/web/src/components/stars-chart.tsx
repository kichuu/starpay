		...
		<div className={cn("transition-opacity", dimmed && "opacity-50")}>
			<div className="relative flex h-[170px] items-end gap-[2px]" aria-hidden>
				...
				{bars.map((bar, index) => {
					const height =
						max > 0
							? Math.max((bar.stars / max) * 120, bar.stars > 0 ? 4 : 0)
							: 0;
					...
					return (
						...
						>
							{active === index && (
								<div className="pointer-events-none absolute bottom-[calc(100%-8px)] z-10 whitespace-nowrap rounded-lg border border-border bg-popover px-2.5 py-1.5 text-center shadow-lg">
									<div className="font-bold text-[13px]">
										★ {formatStars(bar.stars)}
									</div>
