...
function Sidebar({ onNavigate }: { onNavigate?: () => void }) {
	...
	const hosted = useQuery({
		...orpc.payouts.balance.queryOptions(),
		enabled: bot.data === null,
	});
	const botLabel = bot.data
		? `@${bot.data.username}`
		: hosted.data?.platform_bot
			? `via @${hosted.data.platform_bot.username}`
			: "no bot connected";
	...
	return (
		<div className="flex h-full flex-col gap-[22px] px-4 py-[22px]">
			...
			<fieldset className="flex gap-1 rounded-[10px] bg-muted p-1">
				<legend className="sr-only">Mode</legend>
				{(["live", "test"] as const).map((value) => (
					<label
						key={value}
						className={cn(
							"flex-1 cursor-pointer rounded-[7px] py-1.5 text-center font-bold text-[12.5px] capitalize transition-colors has-focus-visible:ring-2 has-focus-visible:ring-ring/50",
							mode !== value &&
								"text-[#6B7C99] hover:text-foreground dark:text-muted-foreground",
							mode === value && value === "live" && "bg-brand text-white",
							mode === value &&
								value === "test" &&
								"bg-test text-test-foreground",
						)}
					>
						<input
							type="radio"
			...
			<div className="mt-auto border-border border-t pt-3.5">
				<AccountMenu
...
function AdminLink({ onNavigate }: { onNavigate?: () => void }) {
	...
	if (!me.data?.platform_admin) return null;
	return (
		<Link
			to="/admin"
			onClick={onNavigate}
			className="flex items-center gap-2.5 rounded-[9px] border border-border border-dashed px-[11px] py-[9px] font-semibold text-[13px] text-muted-foreground hover:bg-muted data-[status=active]:border-brand-line data-[status=active]:bg-brand-soft data-[status=active]:text-brand-strong"
		>
					...
					<Menu.Popup className="w-56 rounded-xl border border-border bg-popover p-1.5 shadow-xl outline-none">
						...
						{(
							...
							>
								...
								{label}
								{theme === value && (
									<span className="ml-auto text-[11px] text-brand">●</span>
								)}
							</Menu.Item>
						))}
						<Menu.Separator className="my-1 h-px bg-border" />
...
function BotStatus() {
	...
	const state: { tone: string; dot: string; text: string } =
		!view && platformBot
			? {
					tone: "border-brand-line bg-brand-soft text-brand-strong",
					dot: "bg-[#12B76A]",
					text: `Payments via @${platformBot.username}`,
				}
			: !view
				? {
	...
	return (
		<Link
			to="/bot"
			className={cn(
				"flex items-center gap-2 whitespace-nowrap rounded-full border px-3 py-[7px] font-semibold text-[12.5px]",
				state.tone,
			)}
		>
...
export function AppShell({ children }: { children: ReactNode }) {
	...
	const page = [...matches]
		.reverse()
		.find((match) => match.staticData?.title)?.staticData;

	return (
		<div className="min-h-svh lg:grid lg:grid-cols-[236px_minmax(0,1fr)]">
			<aside className="hidden border-border border-r bg-sidebar lg:block">
			...
			<main className="flex min-w-0 flex-col">
				{mode === "test" && (
					<div className="bg-test px-4 py-1.5 text-center font-semibold text-[12.5px] text-test-foreground sm:px-7">
						Test mode · Telegram test server. Payments here aren't real.
					</div>
				)}
				<header className="sticky top-0 z-10 flex flex-wrap items-center gap-4 border-border border-b bg-background/90 px-4 py-4 backdrop-blur sm:px-7">
					<button
						type="button"
				...
				<div className="flex flex-col gap-5 px-4 pt-6 pb-14 sm:px-7">
					{children}
				</div>
			</main>
		</div>
