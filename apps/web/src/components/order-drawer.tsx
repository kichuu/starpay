...
function timeline(detail: OrderDetail): TimelineStep[] {
	const steps: TimelineStep[] = detail.timeline.map((entry) => {
		...
		switch (entry.type) {
			...
			case "paid":
				return {
					label: data.late_payment
						? "Payment successful (after expiry)"
						: "Payment successful",
					time,
					state: "done",
				};
			...
			case "expired":
				return {
					label: data.reason === "cancelled" ? "Cancelled" : "Expired",
					time,
					state: "bad",
				};
...
const DOT = {
	...
	pending: "bg-[#D0D5DD] dark:bg-[#2a3a57]",
	bad: "bg-[#F04438]",
};
...
function OrderDrawerBody({ orderId }: { orderId: string }) {
	...
	const detail = useQuery(
		orpc.orders.get.queryOptions({ input: { id: orderId } }),
	);
						...
						<div className="flex flex-col gap-6 px-6 py-5">
							<div className="flex flex-wrap items-center gap-3">
								...
								<div className="text-[12.5px] text-muted-foreground">
									≈ {formatUsd(order.amount)} estimate
								</div>
							</div>

							<section>
								<h3 className="mb-3 font-semibold text-[12px] text-muted-foreground uppercase tracking-[0.04em]">
									Timeline
								</h3>
								...
								{(
									[
										...
										...(order.settlement === "platform"
											? [
													{ label: "Paid via", value: "StarPay's bot" },
													...(order.fee !== null
														? [
																{
																	label: "StarPay fee",
																	value: `★ ${formatStars(order.fee)} · you get ★ ${formatStars(order.amount - order.fee)}`,
																},
															]
														: []),
												]
									...
									>
										...
										<dd className="flex min-w-0 items-center gap-1 text-right font-medium text-[13px]">
							...
							<section>
								<h3 className="mb-2 font-semibold text-[12px] text-muted-foreground uppercase tracking-[0.04em]">
									{detail.data.raw_payment
										? "Raw payment (from Telegram)"
										: "Order object"}
								</h3>
								<pre className="overflow-x-auto rounded-xl bg-[#0B1B2B] p-4 font-mono text-[#CFE0FF] text-[12px] leading-relaxed">
								...
								(confirmRefund ? (
									<div className="rounded-xl border border-[#FDA29B] bg-[#FEF3F2] p-4 dark:border-[#7a2a24] dark:bg-[#2a1215]">
										<div className="font-bold text-[14px]">
											Refund {formatStars(order.amount)} Stars to{" "}
										...
										<p className="mt-1 text-[13px] text-muted-foreground leading-relaxed">
											Telegram returns the Stars to the buyer immediately and
											deducts them from your bot balance. This can't be undone.
										</p>
									...
									>
										Refund <Star className="text-inherit" />{" "}
										{formatStars(order.amount)}
									</Button>
								))}
