...
export function Button({
	...
	return (
		<button
			...
			className={cn(
				"inline-flex cursor-pointer items-center justify-center gap-2 whitespace-nowrap rounded-[10px] font-semibold outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50 [&_svg]:size-4 [&_svg]:shrink-0",
...
export function PanelHeader({
	...
	return (
		<div
			className={cn(
				"flex flex-wrap items-center justify-between gap-3 border-border/70 border-b px-5 py-4",
				className,
			)}
		>
			<div className="min-w-0">
				<h2 className="font-bold text-[14.5px]">{title}</h2>
				{description && (
...
export function Eyebrow({ className, ...props }: ComponentProps<"div">) {
	return (
		<div
			className={cn(
				"font-semibold text-[12px] text-muted-foreground uppercase tracking-[0.04em]",
				className,
			)}
			{...props}
		/>
...
export function Mono({ className, ...props }: ComponentProps<"span">) {
	return (
		<span className={cn("font-mono text-[12.5px]", className)} {...props} />
	);
}
...
export function StarAmount({
	...
	return (
		<span className={cn("whitespace-nowrap font-bold", className)}>
...
export function Pill({
	...
	return (
		<span
			className={cn(
				"inline-block whitespace-nowrap rounded-full border px-2.5 py-0.5 font-bold text-[11.5px]",
				toneStyles[tone],
				className,
			)}
			{...props}
...
export function StatusPill({ status }: { status: string }) {
	const style = ORDER_STATUS[status] ?? {
...
export function Segmented<T extends string>({
	...
	// Real radios: keyboard and screen-reader behaviour comes for free.
	const name = useId();
	return (
		<fieldset
			className={cn(
		...
		>
			...
			{options.map((option) => (
				<label
					key={option.value}
					className={cn(
						"cursor-pointer rounded-lg px-3 py-1.5 font-semibold text-[12.5px] transition-colors has-focus-visible:ring-2 has-focus-visible:ring-ring/50",
						value === option.value
							? "bg-brand text-white"
...
export function Field({
	...
	return (
		<div className="flex flex-col gap-1.5">
			<label htmlFor={htmlFor} className="font-semibold text-[13px]">
				{label}
			</label>
			{children}
			{error ? (
...
export function Toggle({
	...
	return (
		<BaseSwitch.Root
			...
			disabled={disabled}
			aria-label={label}
			className="flex h-6 w-[42px] shrink-0 cursor-pointer items-center rounded-full bg-[#D7DDE7] p-[3px] outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring/50 disabled:opacity-50 data-[checked]:bg-brand dark:bg-[#2a3a57]"
		>
	...
	return (
		<BaseDialog.Root open={open} onOpenChange={onOpenChange}>
			<BaseDialog.Portal>
				...
				<BaseDialog.Popup
					className={cn(
						"fixed top-1/2 left-1/2 z-50 max-h-[calc(100dvh-32px)] w-[calc(100vw-32px)] max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl border border-border bg-card p-6 shadow-2xl transition-all duration-150 data-[ending-style]:scale-95 data-[starting-style]:scale-95 data-[ending-style]:opacity-0 data-[starting-style]:opacity-0",
						className,
					)}
				>
					<div className="mb-5 flex items-start justify-between gap-4">
						<div>
							<BaseDialog.Title className="font-extrabold text-[17px] tracking-[-0.01em]">
								{title}
							</BaseDialog.Title>
							{description && (
			...
			<BaseDialog.Portal>
				...
				<BaseDialog.Popup
					className={cn(
						...
						side === "right"
							? "right-0 max-w-[480px] border-border border-l data-[ending-style]:translate-x-full data-[starting-style]:translate-x-full"
							: "left-0 max-w-[280px] border-border border-r data-[ending-style]:-translate-x-full data-[starting-style]:-translate-x-full",
						className,
					)}
				>
...
export function EmptyState({
	...
	return (
		...
		>
			...
			{children && (
				<div className="max-w-sm text-[13px] text-muted-foreground leading-relaxed">
					{children}
				</div>
			)}
	...
	return (
		...
		>
			{Array.from({ length: rows }, (_, index) => (
				<div
					key={index}
					className="flex items-center gap-4 border-border/60 border-b px-5 py-4"
				>
...
export function CopyButton({
	...
	return (
		<button
			...
			className="inline-flex cursor-pointer items-center gap-1 rounded-md px-1.5 py-1 font-semibold text-[12px] text-muted-foreground hover:bg-muted hover:text-foreground [&_svg]:size-3.5"
		>
...
export function Avatar({
	...
	return (
		<div
			aria-hidden
			className={cn(
				"grid size-[30px] shrink-0 place-items-center rounded-full bg-brand-soft font-bold text-[12px] text-brand-strong",
				className,
			)}
		>
			{text}
...
/**
 * Data tables: real <table> semantics with a CSS-grid layout, so the header and
 * every row share one column template. `state` replaces the body with a loading,
 * empty or error view.
 */
	...
	return (
		<ColumnsContext.Provider value={columns}>
			<div className="overflow-x-auto">
				<table className={cn("block w-full", className)}>
					<caption className="sr-only">{label}</caption>
					<thead className="block">
						<tr
							style={{ gridTemplateColumns: columns }}
							className="grid items-center gap-3 border-border/60 border-b bg-subtle px-5 py-3 text-left font-bold text-[#667085] text-[11.5px] uppercase tracking-[0.04em] dark:text-muted-foreground"
						>
							{head.map((cell, index) => (
								<th key={index} scope="col" className="min-w-0 font-bold">
									{cell}
								</th>
							))}
						</tr>
...
export function DataRow({
	...
	return (
		<tr
			...
			style={{ gridTemplateColumns: columns }}
			className={cn(
				"grid items-center gap-3 border-border/60 border-b px-5 py-3.5 text-[13.5px]",
				interactive &&
