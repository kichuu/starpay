...
function Onboarding() {
  ...
  async function onSubmit(event: FormEvent) {
    ...
    const created = await authClient.organization.create({ name: name.trim(), slug });
  ...
  return (
    <div className="flex min-h-svh items-center justify-center px-4 py-10">
      <div className="w-full max-w-[440px]">
        ...
        <Panel className="p-7">
          <h1 className="text-xl font-extrabold tracking-[-0.02em]">Set up your merchant</h1>
          <p className="mt-1 mb-6 text-[13px] leading-relaxed text-muted-foreground">
            This is the business your buyers pay. You'll connect its Telegram bot and add products next.
          </p>
          <form onSubmit={onSubmit} className="flex flex-col gap-4">
            <Field label="Business or game name" htmlFor="name" error={error ?? undefined}>
              <input
                id="name"
                value={name}
            ...
            <Button type="submit" loading={pending} disabled={!name.trim()} className="w-full">
              Continue
            </Button>
          </form>
        </Panel>
