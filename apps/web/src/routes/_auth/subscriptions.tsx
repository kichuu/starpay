...
export const Route = createFileRoute("/_auth/subscriptions")({
	validateSearch: z.object({
		status: SubscriptionStatus.optional().catch(undefined),
	}),
...
function Subscriptions() {
	...
	const cards = stats.data
		? [
				{
					label: "Active",
					value: formatStars(stats.data.active),
					note: "renewing every 30 days",
				},
				{
					label: "Cancelled",
					value: formatStars(stats.data.cancelled),
					note: "run out at period end",
				},
				{
					label: "Monthly Stars",
					value: (
	...
	return (
		<>
				...
				{cards
					? cards.map((card) => (
							<Panel key={card.label} className="px-[18px] py-4">
								...
								<div className="mt-1.5 font-extrabold text-[26px] tracking-[-0.02em]">
									{card.value}
								</div>
					...
					: Array.from({ length: 3 }, (_, index) => (
							<Shimmer key={index} className="h-[104px] rounded-2xl" />
						))}
			</div>
			...
			<Segmented
				...
				value={status ?? "all"}
				onChange={(value) =>
					navigate({ search: { status: value === "all" ? undefined : value } })
				}
				options={[
					{ value: "all", label: "All" },
			...
			<Panel>
				<DataTable
					...
					head={["Subscriber", "Plan", "Price", "Renews / ends", "Status"]}
					state={
						list.isPending ? (
							<RowsSkeleton />
						) : list.isError ? (
							...
							<EmptyState
								icon={<Repeat />}
								title={
									status ? `No ${status} subscriptions` : "No subscriptions yet"
								}
								action={
									!status && (
										<Link
							...
							>
								Buyers subscribe when they pay for a product of type
								"Subscription". Telegram renews it every 30 days.
							</EmptyState>
						) : undefined
					}
				>
					{list.data?.data.map((subscription) => (
						<DataRow key={subscription.id}>
							<div className="min-w-0">
								...
								<Mono className="text-[11.5px] text-faint">
									{subscription.customer.telegram_user_id}
								</Mono>
							</div>
