		...
		<>
			...
			<Panel>
				<DataTable
					...
					head={["Customer", "Total spent", "Orders", "Last payment", ""]}
					state={
						customers.isPending ? (
							<RowsSkeleton rows={7} />
						) : customers.isError ? (
						...
						return (
							<DataRow key={customer.id}>
								<div className="flex min-w-0 items-center gap-2.5">
									<Avatar
										text={initials(customer.username ?? customer.first_name)}
									/>
									<div className="min-w-0">
										<div className="truncate font-semibold">
											{handle ?? customer.first_name ?? "Telegram user"}
										</div>
								...
								<Link
									...
									className="rounded-lg border border-input px-2.5 py-1 font-semibold text-[12px] hover:bg-muted"
								>
									Orders
								</Link>
							</DataRow>
				...
				{customers.data && customers.data.total > PAGE_SIZE && (
					<div className="flex items-center justify-between px-5 py-3.5 text-[12.5px] text-muted-foreground">
						<div>
							{offset + 1}–{Math.min(offset + PAGE_SIZE, customers.data.total)}{" "}
							of {customers.data.total}
						</div>
