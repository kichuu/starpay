...
function BotPanel() {
	...
	const refresh = useMutation(
		orpc.bot.refresh.mutationOptions({
			onSuccess: (view) => {
				...
				toast.success(
					view.status === "active" ? "Bot is healthy" : "Status updated",
				);
			},
		}),
	);
	...
	if (!bot.data) {
		return <HostedOrConnect onConnected={refreshAll} />;
	}
	...
	const rows = [
		{
			label: "Bot token",
			value:
				view.status === "invalid_token"
					? "Rejected by Telegram"
					: `Valid · ends ${view.token_last4}`,
	...
	return (
		<Panel className="flex flex-col p-5">
			<h2 className="mb-4 font-bold text-[14.5px]">Connected bot</h2>
			<div className="mb-4 flex items-center gap-3">
				<Avatar
					text={initials(view.first_name)}
				...
				<div className="min-w-0">
					<a
						...
						target="_blank"
						rel="noreferrer"
						className="inline-flex items-center gap-1 font-bold text-[15px] hover:underline"
					>
			...
			<dl className="flex flex-col">
				{rows.map((row) => (
					<div
						key={row.label}
						className="flex justify-between gap-4 border-border/60 border-t py-2.5 text-[13px]"
					>
						...
						<dd
							className={
								row.ok
									...
									: "text-right font-semibold text-[#B42318] dark:text-[#f97066]"
							}
						>
							{row.value}
						</dd>
			...
			{view.status === "webhook_error" &&
				view.last_webhook_error?.includes("HTTPS") && (
					<p className="mt-2 rounded-lg bg-muted px-3 py-2 text-[12.5px] text-muted-foreground leading-relaxed">
						Telegram only delivers updates to public HTTPS URLs. Locally, run a
						tunnel to the API and set PUBLIC_API_URL to it, then press Refresh.
					</p>
				)}
			...
			<Dialog
				...
				description="Paste a new token from @BotFather, for this bot or a different one."
			>
				<ConnectBotForm
					onConnected={() => {
						setReplacing(false);
...
/** No bot of their own: payments go through StarPay's bot (if connected), or they connect one. */
function HostedOrConnect({ onConnected }: { onConnected: () => void }) {
	const hosted = useQuery(orpc.payouts.balance.queryOptions());
	const platformBot = hosted.data?.platform_bot;
	const [connecting, setConnecting] = useState(false);

	if (!platformBot) {
		return (
			<Panel className="p-5">
				<h2 className="font-bold text-[14.5px]">Connect your bot</h2>
				<ConnectBotForm onConnected={onConnected} />
			</Panel>
		);
	}
	const plan = hosted.data?.plan;
	return (
		<Panel className="flex flex-col gap-3 p-5">
			<div className="flex items-center justify-between gap-3">
				<h2 className="font-bold text-[14.5px]">Payments through StarPay</h2>
				<Pill tone="success">active</Pill>
			</div>
			<p className="text-[13px] text-muted-foreground leading-relaxed">
				Buyers pay{" "}
				<strong className="text-foreground">@{platformBot.username}</strong>.
				Earnings land in your StarPay balance and are paid out to your TON
				wallet
				{plan
					? ` after a ${plan.hold_days}-day hold, minus StarPay's ${plan.percent_bps / 100}% fee`
					: ""}
				.
			</p>
			<p className="text-[13px] text-muted-foreground leading-relaxed">
				Prefer your own branding and to hold the Stars yourself? Connect your
				own bot: new orders will use it, with no StarPay fee, and you withdraw
				on Fragment.
			</p>
			{connecting ? (
				<ConnectBotForm onConnected={onConnected} />
...
function ConnectBotForm({
	...
	return (
		<form onSubmit={onSubmit} className="mt-3 flex flex-col gap-4">
			<ol className="flex list-decimal flex-col gap-1.5 pl-5 text-[13px] text-muted-foreground leading-relaxed">
				{mode === "test" ? (
					<li>
						Open <strong>@BotFather on Telegram's test server</strong> (a
						separate test account) and create a bot. Test mode only works with
						test-server bots.
					</li>
				) : (
					<li>
				...
				<li>
					StarPay takes over the bot's updates. Don't use this bot for anything
					else.
				</li>
			</ol>
			<Field
				label="Bot token"
				htmlFor="bot-token"
				hint="Stored encrypted. Only the last 4 characters are ever shown."
			>
				<input
					id="bot-token"
					value={token}
...
// Telegram.WebApp.openInvoice(invoice_link) in a Mini App.`;
	...
	return (
		<Panel className="flex flex-col">
				...
				action={
					<div className="flex items-center gap-1">
						...
						<a
							...
							target="_blank"
							rel="noreferrer"
							className="inline-flex items-center gap-1 rounded-md px-1.5 py-1 font-semibold text-[12px] text-brand hover:bg-muted"
						>
			...
			/>
			<pre className="flex-1 overflow-x-auto bg-[#0B1B2B] p-5 font-mono text-[#CFE0FF] text-[12.5px] leading-relaxed">
				{snippet}
			</pre>
		</Panel>
	);
	...
	return (
		<Panel>
				...
				state={
					keys.isPending ? (
						...
						<EmptyState icon={<KeyRound />} title={`No ${mode} keys`}>
							Your server uses a secret key to create orders. Keep it on the
							server, never in an app or website.
						</EmptyState>
					) : undefined
				}
			>
				{active.map((key) => (
					<DataRow key={key.id}>
						...
						<Pill tone={key.mode === "live" ? "success" : "neutral"}>
			...
			<Dialog
				...
				description={`A ${mode} secret key for your server.`}
			>
				<form
					onSubmit={(event) => {
						event.preventDefault();
				...
				>
					<Field
						label="Name"
						htmlFor="key-name"
						hint="Where it's used, e.g. Production server"
					>
						<input
							id="key-name"
							name="name"
