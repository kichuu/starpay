...
export const Route = createFileRoute("/_auth/")({
	validateSearch: z.object({
		range: OverviewRange.optional().catch(undefined),
	}),
	component: Overview,
		...
		<>
			...
			{overview.isError ? (
				...
				<Dashboard
					data={overview.data}
					range={range}
					refreshing={overview.isPlaceholderData}
				/>
			)}
		</>
	);
...
function Dashboard({
	...
	return (
		<>
			<div
				className={cn(
					"grid grid-cols-[repeat(auto-fit,minmax(210px,1fr))] gap-3.5",
					refreshing && "opacity-60",
				)}
			>
				<div className="flex flex-col gap-1.5 rounded-2xl bg-brand px-5 py-[18px] text-white">
					<Eyebrow className="text-[#CFE0FF]">
						Stars revenue · {RANGE_LABEL[range]}
					</Eyebrow>
					{data ? (
						<>
							<div className="flex items-baseline gap-2 font-extrabold text-[30px] tracking-[-0.03em]">
							...
							<div className="text-[#DCE8FF] text-[12.5px]">
								≈ {formatUsd(data.revenue.stars)}{" "}
								<span className="text-[#A9C6FF]">
									· estimate, not a settlement rate
								</span>
							</div>
							{delta(data.revenue.stars, data.revenue.previous) && (
								<div className="mt-0.5 font-semibold text-[#B9FFDC] text-[12.5px]">
									{delta(data.revenue.stars, data.revenue.previous)}
								</div>
							)}
						</>
				...
				<Panel className="flex flex-col gap-1.5 px-5 py-[18px]">
					...
					{data ? (
						<>
							<div className="font-extrabold text-[30px] tracking-[-0.03em]">
								{formatStars(data.payments.count)}
							</div>
						...
						<>
							...
							<div className="h-1.5 overflow-hidden rounded-full bg-muted">
								<div
									...
									style={{ width: `${conversion ?? 0}%` }}
								/>
							</div>
							<div className="text-[12.5px] text-muted-foreground">
								{data.conversion.created} invoices created →{" "}
								{data.conversion.paid} paid
							</div>
						</>
					) : (
						<Shimmer className="h-9 w-20" />
				...
				>
					...
					{data ? (
						<>
							<div
								className={cn(
									"font-extrabold text-[30px] tracking-[-0.03em]",
							...
							<div className="text-[12.5px] text-muted-foreground">
								{data.deliveries.pending} event
								{data.deliveries.pending === 1 ? "" : "s"} waiting to send
							</div>
							<Link
								to="/webhooks"
								className="mt-1 font-semibold text-[12.5px] text-brand hover:underline"
							>
								Open delivery log →
							</Link>
						</>
			...
			<div className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-3.5">
				<Panel className="p-5">
					<div className="mb-4 flex items-baseline justify-between">
						<h2 className="font-bold text-[14.5px]">Stars collected</h2>
			...
			<Panel>
				...
				{!data ? (
					...
					<EmptyState title="No orders yet">
						Orders show up here as soon as your server creates them.
					</EmptyState>
				) : (
					data.recent.map((order) => (
						<Link
							...
							className="flex items-center gap-3.5 border-border/60 border-b px-5 py-3 outline-none last:border-b-0 hover:bg-row-hover focus-visible:bg-row-hover"
						>
							<Mono className="w-[120px] shrink-0 truncate text-[#475467] dark:text-muted-foreground">
								{order.id}
							</Mono>
							<div className="min-w-0 flex-1 truncate font-semibold text-[13.5px]">
								{order.product.name}
							</div>
							<Mono className="hidden text-[#475467] text-[13px] sm:inline dark:text-muted-foreground">
...
function BotHealth({
	...
	pendingDeliveries: number | undefined;
}) {
	...
	const TEXT = {
		ok: "text-[#067647] dark:text-[#47cd89]",
		warn: "text-[#B54708] dark:text-[#fdb022]",
		bad: "text-[#B42318] dark:text-[#f97066]",
	};

	return (
		<Panel className="flex flex-col p-5">
			<h2 className="mb-2 font-bold text-[14.5px]">Bot health</h2>
			{bot.isPending ? (
				<RowsSkeleton rows={3} />
			) : !bot.data ? (
				<EmptyState
				...
				>
					StarPay needs your Telegram bot to create invoices and receive
					payments.
				</EmptyState>
			) : (
				rows.map((row) => (
					<div
						key={row.label}
						className="flex items-center gap-3 border-border/60 border-t py-[11px] first:border-t-0"
					>
						<span
						...
						<div className={cn("font-semibold text-[12px]", TEXT[row.tone])}>
							{row.state}
						</div>
					</div>
				))
...
function Onboarding() {
	...
	const steps = [
		{
			done: status.data.bot_connected,
		...
		{
			done: status.data.has_paid_order,
	...
	return (
		<Panel className="p-5">
			<div className="mb-4 flex flex-wrap items-baseline justify-between gap-2">
				<div>
					<h2 className="font-bold text-[14.5px]">Get set up</h2>
					<p className="mt-0.5 text-[12.5px] text-muted-foreground">
						Four steps to your first Stars payment. Use Test mode while you
						build.
					</p>
				</div>
				...
				{steps.map((step, index) => (
					<li key={step.label}>
						<Link
							to={step.to}
							className={cn(
								"flex h-full items-center gap-3 rounded-xl border px-3.5 py-3 font-semibold text-[13px] transition-colors",
								step.done
						...
						>
							<span
								className={cn(
									"grid size-6 shrink-0 place-items-center rounded-full font-bold text-[12px]",
									step.done
										? "bg-[#12B76A] text-white"
										: "bg-card text-brand ring-1 ring-brand-line",
								)}
							>
								{step.done ? <Check className="size-3.5" /> : index + 1}
							</span>
							<span
								className={step.done ? "line-through decoration-1" : undefined}
							>
								{step.label}
							</span>
						</Link>
